...
import InfoPanel, { InfoPanelFields } from "../components/InfoPanel";
...
const PageWrapper = styled.div<{ modalOpen: boolean }>`
  transition: 300ms padding ease-in-out;
  @media (min-width: ${theme.breakpoints.mobile}) {
    ${({ modalOpen }) =>
      modalOpen ? `padding-left: ${theme.imageInfoPanel.width};` : ``};
  }
`;
...
export default function Home({
  ...
  about: { fields: InfoPanelFields };
  contact: { fields: InfoPanelFields };
}) {
  ...
  let descriptionPanelAnimationTimeout: ReturnType<typeof setTimeout>;
    ...
    <>
      ...
      <div>
        ...
        <PageWrapper modalOpen={descriptionPanelActive}>
          ...
          <ImageList
            images={cmsData.fields.imageList}
            onImageClick={handleImageClick}
          />
          <DescriptionPanel
            active={descriptionPanelActive}
            title={activeImage.title}
            description={activeImage.description}
            onClose={closeDescriptionPanel}
          />
          <InfoPanel
            active={aboutPanelActive}
            fields={about.fields}
            onClose={closeAboutPanel}
          />
          <InfoPanel
            active={contactPanelActive}
