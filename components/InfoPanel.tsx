...
const Panel = styled.div<{ active: boolean }>`
  ...
  ${({ active }) =>
    active
      ...
      : `margin-top: 100vh; visibility: hidden;`}
  display: flex;
...
type ContentType = "text" | "button" | "credit";
type TextAlignment = "left" | "right" | "center";
type ButtonStyle = "primary";

const AlignmentWrap = styled.div<{ alignment: TextAlignment }>`
  ${({ alignment }) => (alignment ? `text-align: ${alignment};` : ``)}
`;

const Button = styled.a<{ buttonStyle: ButtonStyle }>`
  color: ${theme.base.colors.copyLink};
  font-family: sans-serif;
  transition: color 300ms ease-in-out;
  ${({ buttonStyle }) =>
    buttonStyle === "primary"
      ? `font-size: ${theme.cta.primaryFontSize};`
      : `font-size: ${theme.cta.secondaryFontSize};`}
  &:hover {
            ...
            <div key={module.sys.id}>
              ...
              {contentType === "button" && (
                <AlignmentWrap alignment={module.fields.buttonAlignment}>
                  <Button
                    buttonStyle={module.fields.buttonStyle}
                    href={module.fields.buttonLink}
                    target={
                      module.fields.openInANewWindow ? "_blank" : undefined
                    }
                    rel={
                      module.fields.openInANewWindow
                        ? "noopener noreferrer"
                        : undefined
                    }
                  >
